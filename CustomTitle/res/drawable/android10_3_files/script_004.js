var _nc_wp_atit_rc2_clicked = function (url) {
	new Image().src = 'http://connect.newzia.jp/channels/127/clicked'
...
;
var _nc_wp_atit_rc2_callback = function (response) {
	(function (contents_length) {
	new Image().src = 'http://connect.newzia.jp/channels/127/viewed'
	...
	var newziaConnect = document.getElementById('newzia_connect_wp_atit_rc2_');
...
var rc2temp = '';
rc2temp += "<div id=\"newzia_connect_wp_atit_rc2\" class=\"btmlist\"><h3>ホワイトペーパー<small>（<a href=\"http://techtarget.itmedia.co.jp/tt/membership/\">TechTargetジャパン</a>）</small></h3><ul>";

for (i = 0; i < response.length; i++) {
	rc2temp += "<li><a onclick=\"var s=s_gi('itmediacojpglobal'); s.eVar21='" + getMedianame() + "'; s.tl(this,'o','" + getMedianame() +"');\" href=\"" + response[i].url + "\" onmousedown=\"_nc_wp_atit_rc2_clicked(this.href)\">" + response[i].title + "</a></li>";
}

rc2temp += "</ul></div>";
newziaConnect.innerHTML = rc2temp;
...
;
		...
		script.src = 'http://connect.newzia.jp/channels/127/' + md5.hex(document.URL) + '/contents.json?'
			 + 'url=' + encodeURIComponent(document.URL)
			 + '&ref=' + encodeURIComponent(document.referrer)
			 ;
	};

	if (!document.getElementById('newzia_connect_wp_atit_rc2_')) {
		document.write('<div id="newzia_connect_wp_atit_rc2_"></div>');
